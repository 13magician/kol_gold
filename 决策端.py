...
def 接收情报接口():
    try:
        ...
        # [新增] 读取历史上下文 (最近2条)
        历史记录 = db.读取_最近聊天记录(KOL名称, limit=2)

        # [Debug] 打印发送给AI的文本内容
        实际发送文本 = f"KOL名称: {KOL名称}\n原始消息:\n{原始内容}"
        db.带时间的日志打印(f"📤 [Debug-AI输入] 完整文本:\n{实际发送文本}")
        if 历史记录:
            db.带时间的日志打印(f"📜 [Debug-AI输入] 历史上下文: {json.dumps(历史记录, ensure_ascii=False)}")

        # 1. 呼叫 AI 大脑分析 (增加计时)
        开始时间 = time.time()
        是否信号, 分析结果, AI原始回复 = AI核心.分析信号(KOL名称, 原始内容, 图片列表, 历史记录)
        耗时 = time.time() - 开始时间

        # [新增] 记录本次交互到数据库
        db.写入_聊天记录(KOL名称, 原始内容, AI原始回复, 是否信号)
        ...
        if 是否信号 and 分析结果:
            ...
            # === 🔥 分支 C: 止盈/保本指令 ===
            if 方向 == "止盈":
                打印器.决策_平仓令(KOL名称, 品种) # 复用平仓打印
                db.带时间的日志打印(f"🥂 [收到止盈令] {KOL名称} 提示 {品种} 止盈/保本")
                父ID = db.写入_父信号(KOL名称, 品种, "止盈", "市价", 0, {})
                db.写入_子命令(父ID, KOL名称, 品种, "止盈", 0, 0, 0, 0)
                return jsonify({"状态": "成功", "类型": "止盈指令"}), 200
