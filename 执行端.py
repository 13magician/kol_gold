...
class 执行指挥官:
    ...
    def 执行_清仓操作(self, kol_name, symbol):
        ...
        # 1. 处理 active_positions (包括 Bot的所有单 + 手动持仓)
        try:
            所有持仓 = db.读取_所有活跃持仓()
            for pos in 所有持仓:
                if pos['kol_name'] == kol_name:
                    if symbol == "ALL" or pos['symbol'] == symbol:
                        ...
                        已处理Tickets.add(ticket)
                        
                        # 尝试平仓
                        res, msg = self.MT5.执行平仓(ticket)
                        if res:
                            db.带时间的日志打印(f"✅ [清仓] 平仓成功 Ticket:{ticket}")
                            db.移除_持仓记录(ticket)
                        else:
                            # 如果平仓失败，可能是挂单，尝试撤单
                            res2, msg2 = self.MT5.撤销挂单(ticket)
                            if res2:
                                db.带时间的日志打印(f"✅ [清仓] 撤单成功 Ticket:{ticket}")
                                db.移除_持仓记录(ticket)
                            else:
                                db.带时间的日志打印(f"⚠️ [清仓] 操作失败 Ticket:{ticket} | 平仓:{msg} | 撤单:{msg2}")
        except Exception as e:
            db.带时间的日志打印(f"❌ [清仓-持仓] 异常: {e}")

        # 2. 处理 command_queue 中的挂单 (主要是手动挂单，也包含Bot挂单)
        try:
            挂单列表 = db.查询_KOL挂单(kol_name, symbol)
            for order in 挂单列表:
                ticket = int(order['mt5_ticket'])
                if ticket in 已处理Tickets: continue
                
                res, msg = self.MT5.撤销挂单(ticket)
                if res: db.带时间的日志打印(f"✅ [清仓] 撤单成功 Ticket:{ticket}")
                else: db.带时间的日志打印(f"⚠️ [清仓] 撤单失败 Ticket:{ticket} | {msg}")
        except Exception as e:
            db.带时间的日志打印(f"❌ [清仓-挂单] 异常: {e}")
